...
def get_stochastic_network_move_x(session, input_layer, output_layer, board_state, side,
    ...
    #print("\n")
    #time.sleep(3)
    quarter = int(random.randint(-1,3))
    direction = int(math.pow(-1, random.randint(0,2)))
    move = np.append(move, quarter)
    move = np.append(move, direction)
    print(move)
    return move
...
if __name__ == '__main__':
    ...
    input_layer, output_layer, variables = create_network(36, ((100), (100), (100)))

    load_network(session, variables, "testnetwork.p")

    print(variables)
    # example of playing a game
    play_game(random_player, get_stochastic_network_move_x, session, log=True, board_size=6, winning_length=5) #board_size=10, winning_length=4